...
#               self.draw_figure_y1y2( m1_data, m2_data, abs( complex( dm_re, dm_im ) )/ds,'source_m', 'measure', 'dm/ds' )
    ...
    def m4p_lk_w_PS(
        ...
        '''
            ...
            self.hidden_4p_lock_w_PS( file_path, s_dac, s_dacn, k_s, k_m, lk, sweep, s_sleep ) #fill in

            if s.sweep_back:
                # This one wont work - fix it for hidden 4p first!
                self.pcolor = 'ro'
                
                self.hidden_4p_lock_w_PS( file_path, s_dac, s_dacn, k_s, k_m, lk, sweep[ ::-1 ], s_sleep )           
            ...
            if repeat != 0:
                ...
                for i in range( 0, repeat, 1 ):
                    ...
                    self.hidden_4p_lock_w_PS( file_path, s_dac, s_dacn, k_s, k_m, lk, sweep, s_sleep )
