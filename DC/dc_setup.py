...
measure_line = lambda rack_config, amplification_on_module, c4, c2 : rack_config.format( amplification_on_module ) + '\n\t\tp4 = {}\n\t\tp2 = {}'.format( c4, c2 )  
...
#        return self.sweep_or_bias
    ...
    def line( self ):
        ...
        return G_source_line( self.rack0, self.name[ 1 ], self.dac, self.V_per_V, self.c1 )    
...
class Measure:
    ...
    '''
    def __init__( self, rack, amplification_on_module, unit = 'mV', c4 = '', c2 = '' ):
        
        self.rack0 = rack
        self.amplification_on_module = amplification_on_module 
        self.unit = unit
    ...
    def column( self ):
        ...
        if self.name[ 0 ] == 'I':
            ...
            do_times = 1/( self.amplification_on_module*additional_amplification*1e6 )
    ...
    def line( self ):
        ...
        return measure_line( self.rack0, self.amplification_on_module, self.c4, self.c2 )  
...
def CreateFilePath(filename: str, filefolder: str, filepath: str, number=False):
    """
    It looks in the given subfolder path and finds the max Meas_ number in the folder,
    and automatically assigns number = max number + 1 if number=False.
    If the file with the max Meas_ number has no data inside, it asks you if you want to overwrite that file without creating a new one.
    You can input the Meas_ number manually, and the function will check if it already exists.
    """
    ...
    if not number:
        ...
        if NumberExists(path, number):
            print("WARNING: File number already exists!\n")
            while True:
...
#----------------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------------------------------
print( 'DC_setup.py lib imported!' )