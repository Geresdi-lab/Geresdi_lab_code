...
class circlefit(object):
    ...
    def _fit_circle(self,z_data, refine_results=True, newton = True):
        ...
        if newton:
            x0 = spopt.newton(func, 0., fprime=d_func)
            U,s,Vt = solve_eq_sys(x0,M)
        ...
        #xc,yc= 0.15485685349825122,-0.4538323235731014
        #print(f"r0= {r0}, xc,yc= {xc},{yc}")
    ...
    def _fit_delay(self,f_data,z_data,delay=0.,maxiter=1000):
        def residuals(p,x,y):
            phasedelay = p
    ...
    def _fit_delay_alt_bigdata(self,f_data,z_data,delay=0.,maxiter=1000):
        def residuals(p,x,y):
            phasedelay = p
